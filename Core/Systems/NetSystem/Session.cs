...
namespace Core.Systems.NetSystem
{
    public class Session : TcpSession
    {
        ...
        public Session(TcpServer server, HandlerProvider provider, ILogger logger) : base(server)
        {
            _provider = provider;
            _logger = logger;
        }
...
#pragma warning restore CA1031 // Do not catch general exception types
        ...
        public Session SendAsync(WriterPacket pw) => SendAsync(UtilsPacket.Pack(pw));

        public new Session SendAsync(byte[] bytes)
        {
