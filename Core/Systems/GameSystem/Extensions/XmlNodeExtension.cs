...
namespace Core.Systems.GameSystem.Extensions
{
    internal static class XmlNodeExtension
    {
        ...
        internal static float GetSingle(this XmlNode xml, string xpath) =>
            float.Parse(xml.SelectSingleNode(xpath).Attributes.GetNamedItem("value").Value);

        internal static uint GetUInt32(this XmlNode xml, string xpath) =>
            uint.Parse(xml.SelectSingleNode(xpath).Attributes.GetNamedItem("value").Value);

        internal static ushort GetUInt16(this XmlNode xml, string xpath) =>
            ushort.Parse(xml.SelectSingleNode(xpath).Attributes.GetNamedItem("value").Value);

        internal static byte GetByte(this XmlNode xml, string xpath) =>
            byte.Parse(xml.SelectSingleNode(xpath).Attributes.GetNamedItem("value").Value);

        internal static bool GetBool(this XmlNode xml, string xpath) =>
            bool.Parse(xml.SelectSingleNode(xpath).Attributes.GetNamedItem("value").Value);

        internal static T[] GetSplitted<T>(this XmlNode xml, string xpath, Func<string, T> func) =>
            xml.SelectSingleNode(xpath).Attributes.GetNamedItem("value").Value.Split(',').Select(func).ToArray();
