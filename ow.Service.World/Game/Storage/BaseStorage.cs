...
namespace ow.Service.District.Game.Storage
{
    public abstract class BaseStorage : List<StorageItem?>
    {
        ...
        public BaseStorage(IEnumerable<ItemModel> models, BinTable tables, ushort maxCapacity) : base(GetItems(models, tables, maxCapacity))
        {
        }

        private static IEnumerable<StorageItem?> GetItems(IEnumerable<ItemModel> models, BinTable tables, ushort maxCapacity)
        {
