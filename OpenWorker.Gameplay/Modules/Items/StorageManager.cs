    ...
    ReadOnlyCollection<BankSlotExtendRow> bank)
{
    ...
    {
        ref var component = ref world.Get<StorageComponent>(player);
        var storage = component.Collection.First(x => world.Get<StorageGroupComponent>(x).Group == type);
        ref var grade = ref world.Get<StorageGradeComponent>(storage);

        var info = GetGradeUpInfo(type, grade.Level + 1);

        ref var session = ref world.Get<ServerSessionComponent>(player);
        ref var content = ref world.Get<StorageContentComponent>(storage);
        ...
        switch (info.ConditionType)
        {
            ...
            case SlotExtendPaymentType.Item:
            {
                ...
                if (count > 0)
                {
                    ...
                    session.Send(new ItemReduceResponse { Storage = type, Index = index, Count = count });
                }
                ...
                else
                {
                    ...
                    session.Send(new ItemBreakResponse { Storage = type, Index = index });
                }
        ...
        session.Send(new ItemAddSlotResponse
        {
            Storage = type,
            ExtendStep = grade.Level,
            SlotCount = info.TotalSlot
        });
    }
    ...
    public StorageTryAddResult TryAdd(Entity player, Entity item, short count)
    {
        ...
        if (index == -1)
        {
            return new StorageTryAddResult(false, Entity.Null, []);
        }

        content.SlotList[index] = item;

        return new StorageTryAddResult(true, storage, [new StorageTryAddSlotInfo((short)index, count)]);
    }
    ...
    public void Break(Entity player, StorageGroup group, short index, int count)
    {
        ...
        if (targetCount <= 0)
        {
            ...
            session.Send(new ItemBreakResponse { Storage = group, Index = index });
        }
        ...
        else
        {
            ...
            session.Send(new ItemReduceResponse { Storage = group, Index = index, Count = targetCount });
        }
    }
    ...
    public void Divide(Entity player, StorageGroup srcGroup, StorageGroup destGroup, short srcIndex, short destIndex, short count)
    {
        ...
        ref var srcContent = ref world.Get<StorageContentComponent>(srcStorage);

        var srcItem = srcContent.SlotList[srcIndex];

        // WTF: Source item does not exist
        ...
        session.Send(new ItemDivideResponse
        {
            ...
            DestIndex = destIndex,
            DestItem = ItemDtoFactory.FromEntity(world, item)
        });
    }
    ...
    public void Combine(Entity player, StorageGroup srcGroup, StorageGroup destGroup, short srcIndex, short destIndex, short count)
    {
        ...
        session.Send(new ItemCombineResponse
        {
            ...
            SrcStorage = srcGroup,
            SrcIndex = srcIndex,
            SrcItem = ItemDtoFactory.FromEntity(world, srcItem),
            DestActor = actor,
            DestStorage = destGroup,
            DestIndex = destIndex,
            DestItem = ItemDtoFactory.FromEntity(world, destItem),
        });
    }
}
