...
public readonly struct MovementMoveRequest(BinaryReader reader) : IRequestHotspotMessage
{
    private const GroupOpcode Group = GroupOpcode.Move;
    private const MoveOpcode Command = MoveOpcode.Move;
    ...
    public byte byRunBit { get; } = reader.ReadByte();
    public float fPitch { get; } = reader.ReadSingle();
    public float fMoveSpeed { get; } = reader.ReadSingle();
    public byte byChangeMotion { get; } = reader.ReadByte();
