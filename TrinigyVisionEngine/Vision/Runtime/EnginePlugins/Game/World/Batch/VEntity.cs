...
namespace TrinigyVisionEngine.Vision.Runtime.EnginePlugins.Game.World.Batch
{
    ...
    public abstract class VEntity
    {
        ...
        {
            Id = int.Parse(xml.SelectSingleNode("m_ID").Attributes.GetNamedItem("value").Value);
            Generate = int.Parse(xml.SelectSingleNode("m_iGenerateID").Attributes.GetNamedItem("value").Value);
            ...
            var posTopLeft = xml.SelectSingleNode("m_vPosTopLeft").Attributes.GetNamedItem("value").Value.Split(',').Select(float.Parse).ToArray();
            PosTopLeft = new Vector3(posTopLeft[0], posTopLeft[1], posTopLeft[2]);

            var posBottomRight = xml.SelectSingleNode("m_vPosBottomRight").Attributes.GetNamedItem("value").Value.Split(',').Select(float.Parse).ToArray();
            ...
            var size = xml.SelectSingleNode("m_vSize").Attributes.GetNamedItem("value").Value.Split(',').Select(float.Parse).ToArray();
            ...
            var colorLdr = xml.SelectSingleNode("m_ColorLDR").Attributes.GetNamedItem("value").Value.Split(',').Select(byte.Parse).ToArray();
